...
- Los archivos XXXERR.csv y XXXLOG.csv solo se crean si se encontraron errores en el archivo original, es decir, si en un archivo no se detecta ningún error, so se crearán XXXERR.csv ni XXXLOG.csv. Por otro lado los archivos XXXOK.csv se crean siempre, en caso de que no se detecten errores en un archivo igual se creará XXXOK.csv, solo que será idéntico al original. Y obviamente en caso de que se detecten errores XXXOK.csv será la versión corregida.
